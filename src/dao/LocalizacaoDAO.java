...
public class LocalizacaoDAO {
    ...
    public void Salvar(Localizacao obj) {
        try {
            //1º Criar o SQL
            String sql = "insert into Localizacao_Conteineres (Id_conteiner, data, localizacao)"
                    + "values(?,?,?)";
    ...
    public void Editar(Localizacao obj) {
        try {
            //1º Criar o SQL
            String sql = "update Localizacao_Conteineres set id_localizacao =?, Id_conteiner=?, data=?, localizacao=?";
            //2ºPreparar a conexão SQL para se conectar com o Banco
            PreparedStatement stmt = conn.prepareStatement(sql);
            //3º Inserir os dados
            stmt.setInt(1, obj.getConteiner().getId_container());
    ...
    public List<Localizacao> Listar() {
        ...
        try {
            String sql = "Select * from Localizacao_Conteineres";
            PreparedStatement stmt = conn.prepareStatement(sql);
            ResultSet rs = stmt.executeQuery();
